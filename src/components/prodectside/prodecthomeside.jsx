...
const FALLBACK_IMAGE = 'https://dummyimage.com/400x300/fff/aaa&text=Image+Not+Available';
...
function Prodecthomepage() {
  ...
  const oneByoneOrderFunc = async (cartid) => {
    try {
      const token = localStorage.getItem('access');
      const urldata = await axios.get(`${apiUrl}/orders/UseraddressGet`, { 
        headers: { Authorization: `Bearer ${token}`} 
      });
            ...
            <nav className="relative z-10 py-4 px-6 backdrop-blur-sm bg-white/90 shadow-md">
              <div className="max-w-7xl mx-auto flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <button onClick={logoutfunc} className="px-5 py-2.5 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 text-white hover:from-amber-600 hover:to-amber-700 transition-all duration-300 shadow-md hover:shadow-lg font-medium">Logout</button>
                </div>
              </div>
            </nav>
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredProducts.map((product) => (
                  ...
                  >
                    <div onClick={() => setSelectedProduct(product)} className="relative h-72 overflow-hidden cursor-pointer">
                      <img 
