  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden fixed top-4 left-4 z-50 p-2 bg-indigo-700 rounded-md text-white"
      >
        {isOpen ? 'Close' : 'Menu'}
      </button>
      ...
      {/* Sidebar */}
      <div
        className={`bg-gradient-to-b from-indigo-900 to-purple-800 text-white w-64 min-h-screen p-6 flex flex-col fixed md:relative z-40 transition-all duration-300 ease-in-out 
              ...
              {homeside.map(product => (
                <li key={product.id}>
                  <div className="flex items-center justify-between p-2 rounded-lg hover:bg-indigo-700 transition">
                    <Link to={`/${product.name}`} onClick={() => setIsOpen(false)}>
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-2">
              {categories.map(category => (
                <li key={category.id}>
                  <div className="flex items-center justify-between p-2 rounded-lg hover:bg-indigo-700 transition">
                    <Link to={`/${category.name}`} onClick={() => setIsOpen(false)}>
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-2">
              {userside.map(us => (
                <li key={us.id}>
                  <div className="flex items-center justify-between p-2 rounded-lg hover:bg-indigo-700 transition">
                    <Link to={`/${us.name}`} onClick={() => setIsOpen(false)}>
                      <span>{us.name}</span>
                    </Link>
                  </div>
                </li>
